...
INSERT INTO cs_customers(full_name, email, phone, password) VALUES
  ('FROM PHP FORM'),
  ('FROM PHP FORM'),
  ('FROM PHP FORM'),
  ('FROM PHP FORM');

DELETE from cs_customers WHERE cs_customer_id = 'FROM LOGGED IN USER INFO';
...
INSERT INTO cs_admins(full_name, position, employee_id_num) VALUES
  ('FROM PHP FORM'),
  ('FROM PHP FORM'),
  ('FROM PHP FORM');

DELETE from cs_admins WHERE cs_admin_id = 'FROM LOGGED IN USER INFO';

CREATE TABLE cs_addresses (
  cs_address_id MEDIUMINT NOT NULL AUTO_INCREMENT,
  cs_customer_id MEDIUMINT UNSIGNED NOT NULL,
  ...
  INDEX ind_customer_id(cs_customer_id),
  CONSTRAINT fk_customer_id FOREIGN KEY (cs_customer_id) REFERENCES cs_customers(cs_customer_id) ON DELETE CASCADE ON UPDATE CASCADE
)ENGINE = InnoDB;

INSERT INTO cs_addresses(cs_customer_id, type, address_street1, address_street2, city, state, zip) VALUES
...
DELETE from cs_addresses WHERE cs_address_id = 'FROM SELECTED PHP ELEMENT (RADIO)';

CREATE TABLE cs_address_types (
  cs_address_type_id TINYINT NOT NULL AUTO_INCREMENT,
  type VARCHAR(9) NOT NULL,
  PRIMARY KEY(cs_address_type_id)
)ENGINE = InnoDB;

INSERT INTO cs_address_types(type) VALUES
  ('business'),
  ('home'),
  ('apartment');

CREATE TABLE cs_credit_cards (
  cs_credit_card_id MEDIUMINT NOT NULL AUTO_INCREMENT,
  cs_customer_id MEDIUMINT UNSIGNED NOT NULL,
  ...
  INDEX ind_customer_id(cs_customer_id),
  CONSTRAINT fk_customer_cc_id FOREIGN KEY (cs_customer_id) REFERENCES cs_customers(cs_customer_id) ON DELETE CASCADE ON UPDATE CASCADE
)ENGINE = InnoDB;

INSERT INTO cs_credit_cards(cs_customer_id, cs_address_id, cs_credit_card_type_id, credit_card_number, exp_month, exp_year) VALUES
  ('LOGGED IN USER INFO'),
  ('FROM PHP FORM'),
  ('FROM PHP FORM'),
  ('FROM PHP FORM'),
...
DELETE from cs_credit_cards WHERE cs_credit_card_id = 'FROM SELECTED PHP ELEMENT (RADIO)';

CREATE TABLE cs_credit_card_types (
  cs_credit_card_type_id TINYINT NOT NULL AUTO_INCREMENT,
  card_type VARCHAR(10) NOT NULL,
  PRIMARY KEY(cs_credit_card_type_id)
)ENGINE = InnoDB;

INSERT INTO cs_credit_card_types(card_type) VALUES
  ('Visa'),
  ('MasterCard'),
  ('American Express'),
  ('Discover');

CREATE TABLE cs_carts (
  cs_case_id MEDIUMINT NOT NULL AUTO_INCREMENT,
  ...
  INDEX ind_customer_id(cs_customer_id),
  CONSTRAINT fk_customer_cart_id FOREIGN KEY (cs_customer_id) REFERENCES cs_customers(cs_customer_id) ON DELETE CASCADE ON UPDATE CASCADE
)ENGINE = InnoDB;

INSERT INTO cs_carts(cs_card_id, cs_customer_id, cs_clothing_id, cs_clothing_size_id, cs_clothing_color_id, cs_clothing_price_id, quantity) VALUES
...
DELETE from cs_carts WHERE cs_case_id = 'FROM (MULTIPLE) SELECTED ITEMS FROM CART VIEW BASED ON USER INPUT';

CREATE TABLE cs_transactions (
  cs_transaction_id MEDIUMINT NOT NULL AUTO_INCREMENT,
  cs_customer_id MEDIUMINT UNSIGNED NOT NULL,
  ...
  INDEX ind_customer_id(cs_customer_id),
  CONSTRAINT fk_customer_trans_id FOREIGN KEY (cs_customer_id) REFERENCES cs_customers(cs_customer_id) ON DELETE CASCADE ON UPDATE CASCADE
)ENGINE = InnoDB;

INSERT INTO cs_transactions(customer_id, shipping_type_id, shipping_cost, shipping_address_id, credit_card_id) VALUES
  ('LOGGED IN USER INFO'),
  ('FROM PHP FORM'),
  ('FROM PHP FORM'),
  ('FROM PHP FORM'),
...
CREATE TABLE cs_shipping_types (
  cs_shipping_type_id TINYINT NOT NULL AUTO_INCREMENT,
  carrier VARCHAR(5) NOT NULL,
  type VARCHAR(9) NOT NULL,
  PRIMARY KEY(cs_shipping_type_id)
)ENGINE = InnoDB;

INSERT INTO cs_shipping_types(carrier, type) VALUES
  ('UPS', 'Ground'),
  ('UPS', 'Second Day'),
  ('USPS', 'Standard'),
  ('USPS', 'Expedited'),
...
CREATE TABLE cs_clothing_items (
  cs_clothing_id MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT,
  cs_item_id TINYINT NOT NULL,
  ...
  date_added TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY(cs_clothing_id)
)ENGINE = InnoDB;

INSERT INTO cs_clothing_items(clothing_type_id, clothing_description_id, clothing_size_id, clothing_color_id, clothing_photo, clothing_price, clothing_stock_quantity, weight) VALUES

INSERT INTO `cs_clothing_items` (`cs_clothing_id`, `cs_item_id`, `clothing_type_id`, `clothing_title`, `clothing_description_id`, `clothing_color_id`, `clothing_photo`, `clothing_price`, `clothing_sale_price`, `qty_xxs`, `qty_xs`, `qty_s`, `qty_m`, `qty_l`, `qty_xl`, `qty_xxl`, `weight`, `date_added`) VALUES
/* Coat 01 */
/* Black */
...
DELETE from cs_clothing_items WHERE cs_clothing_id = 'FROM SELECTED PHP ELEMENT (DROPDOWN OR DELETE BUTTON VISIBLE TO ADMINS)';

CREATE TABLE cs_clothing_types (
  cs_clothing_type_id TINYINT NOT NULL AUTO_INCREMENT,
  item_type VARCHAR(9) NOT NULL,
  PRIMARY KEY(cs_clothing_type_id)
)ENGINE = InnoDB;

INSERT INTO cs_clothing_types(item_type) VALUES
  ('coat'),
  ('shirt'),
  ('tshirt'),
  ('pant'),
...
CREATE TABLE cs_clothing_descriptions (
  cs_clothing_description_id MEDIUMINT NOT NULL AUTO_INCREMENT,
  cs_clothing_type_id MEDIUMINT NOT NULL,
  description MEDIUMTEXT NOT NULL,
  PRIMARY KEY(cs_clothing_description_id)
)ENGINE = InnoDB;

INSERT INTO cs_clothing_descriptions(cs_clothing_type_id, description) VALUES
  ('FROM PHP FORM'),
  ('FROM PHP FORM');

DELETE from cs_clothing_descriptions WHERE cs_clothing_description_id = 'FROM SELECTED PHP ELEMENT (DROPDOWN OR DELETE BUTTON VISIBLE TO ADMINS)';

CREATE TABLE cs_clothing_sizes (
  cs_clothing_size_id TINYINT NOT NULL AUTO_INCREMENT,
  size VARCHAR(8) NOT NULL,
  PRIMARY KEY(cs_clothing_size_id)
)ENGINE = InnoDB;

INSERT INTO cs_clothing_sizes(size) VALUES
  ('XX-Small'),
  ('X-Small'),
  ('Small'),
  ('Medium'),
...
CREATE TABLE cs_clothing_colors (
  cs_clothing_color_id TINYINT NOT NULL AUTO_INCREMENT,
  color VARCHAR(12) NOT NULL,
  PRIMARY KEY(cs_clothing_color_id)
)ENGINE = InnoDB;

INSERT INTO cs_clothing_colors(color) VALUES
  ('blue'),
  ('red'),
  ('yellow'),
  ('plaid_blue'),
...
CREATE TABLE cs_clothing_sales (
  cs_sale_id MEDIUMINT NOT NULL AUTO_INCREMENT,
  cs_clothing_id MEDIUMINT NOT NULL,
  ...
  PRIMARY KEY(cs_sale_id)
)ENGINE = InnoDB;

INSERT INTO cs_clothing_sales(cs_clothing_id, sale_price) VALUES
  ('FROM PHP FORM'),
  ('FROM PHP FORM');

DELETE from cs_clothing_sales WHERE cs_sale_id = 'FROM PHP DROPDOWN SELECTION MADE BY ADMIN';
